...
class Agent:
    ...
    def run(self, is_training=True, render=False):
        ...
        for episode in itertools.count():
            state, _ = env.reset()
            state = torch.tensor(state, dtype=torch.float, device=device) / 100
            ...
            while (not terminated and episode_reward < self.reward_threshold):
                ...
                next_state = torch.tensor(next_state, dtype=torch.float, device=device) / 100
            ...
            if is_training:
                ...
                if episode_reward > best_reward:
                    with open(self.LOG_FILE, "a") as f:
                        f.write(f"{episode+1},{episode_reward}\n")
